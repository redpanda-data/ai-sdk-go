...
func TestModelGPT51_Definition(t *testing.T) {
	t.Parallel()

	provider, err := NewProvider("sk-test-key")
	require.NoError(t, err)

	// Verify GPT-5.1 is in supported models list
	models := provider.Models()
	...
	assert.Contains(t, modelNames, ModelGPT5_1, "Should include gpt-5.1")

	// Verify base model definition
	model, err := provider.NewModel(ModelGPT5_1)
	require.NoError(t, err)
	assert.Equal(t, ModelGPT5_1, model.Name())

	// Verify capabilities
	def := supportedModels[ModelGPT5_1]
	assert.True(t, def.Capabilities.Reasoning, "Should support reasoning")
	assert.True(t, def.Capabilities.Vision, "Should support vision")
	assert.True(t, def.Capabilities.Audio, "Should support audio")
	assert.True(t, def.Capabilities.Tools, "Should support tools")
	assert.Equal(t, 272000, def.Constraints.MaxInputTokens)
	assert.Equal(t, 128000, def.Constraints.MaxOutputTokens)
}

func TestModelGPT51_Constraints(t *testing.T) {
	t.Parallel()

	provider, err := NewProvider("sk-test-key")
	require.NoError(t, err)

	// GPT-5.1 should support reasoning parameters
	_, err = provider.NewModel(ModelGPT5_1, WithReasoningEffort(ReasoningEffortLow))
	require.NoError(t, err)

	_, err = provider.NewModel(ModelGPT5_1, WithReasoningEffort(ReasoningEffortMedium))
	require.NoError(t, err)

	// Should have same temperature constraints as GPT-5
	_, err = provider.NewModel(ModelGPT5_1, WithTemperature(2.0))
	require.NoError(t, err)

	_, err = provider.NewModel(ModelGPT5_1, WithTemperature(2.5))
	require.Error(t, err)
	assert.Contains(t, err.Error(), "out of range")

	// Should reject mutually exclusive params
	_, err = provider.NewModel(ModelGPT5_1, WithTemperature(0.7), WithTopP(0.9))
	require.Error(t, err)
	assert.Contains(t, err.Error(), "cannot use")
}

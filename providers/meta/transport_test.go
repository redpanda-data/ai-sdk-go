...
func TestMuseSparkResponsesTransport(t *testing.T) {
	...
	for _, budget := range []int{0, 1024, 32768} {
		for _, streaming := range []bool{false, true} {
			t.Run(fmt.Sprintf("stream=%t/budget=%d", streaming, budget), func(t *testing.T) {
				t.Parallel()
				...
				server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					if budget == 0 {
						assert.Nil(t, body.MaxOutputTokens)
					} else if assert.NotNil(t, body.MaxOutputTokens) {
						assert.Equal(t, budget, *body.MaxOutputTokens)
					}
				...
				opts := []openai.Option{openai.WithReasoningEffort(openai.ReasoningEffortMax)}
				if budget != 0 {
					opts = append(opts, openai.WithMaxTokens(budget))
				}

				m, err := p.NewModel(ModelMuseSpark13, opts...)
				require.NoError(t, err)

...
// TestRunBlocking_Success verifies blocking API returns result.
func TestRunBlocking_Success(t *testing.T) {
	t.Parallel()

	store := session.NewInMemoryStore()

	expectedUsage := &llm.TokenUsage{
		InputTokens:  10,
		OutputTokens: 20,
		TotalTokens:  30,
	}
	...
	// Execute with blocking API
	finishReason, usage, err := r.RunBlocking(ctx, "", "test-session", userMsg)

	// Assert
	require.NoError(t, err)
	assert.Equal(t, agent.FinishReasonStop, finishReason)
	require.NotNil(t, usage)
	assert.Equal(t, expectedUsage.TotalTokens, usage.TotalTokens)
}

// TestRunBlocking_Error verifies blocking API returns errors.
func TestRunBlocking_Error(t *testing.T) {
	t.Parallel()
	...
	ag := &mockAgent{
		...
		runFunc: func(invCtx *agent.InvocationContext) iter.Seq2[agent.Event, error] {
			return func(yield func(agent.Event, error) bool) {
				// Emit error
				yield(nil, errors.New("execution failed"))
	...
	// Execute
	_, _, err = r.RunBlocking(ctx, "", "test-session", userMsg)
...
// TestRunBlocking_NoCompletionEvent verifies error when no completion event.
func TestRunBlocking_NoCompletionEvent(t *testing.T) {
	t.Parallel()
	...
	ag := &mockAgent{
		...
		runFunc: func(invCtx *agent.InvocationContext) iter.Seq2[agent.Event, error] {
			return func(yield func(agent.Event, error) bool) {
				// Emit status event but no completion event
				...
				// Stream ends without InvocationEndEvent
			}
		},
	}
	...
	// Assert
	require.Error(t, err)
	assert.Contains(t, err.Error(), "no completion event")
}
...
// collectEventsWithErrors collects all events from an iterator, including events before errors.
// Returns both the collected events and the first error encountered.
// Use this helper for tests that explicitly verify error handling.
func collectEventsWithErrors(t *testing.T, iter func(func(agent.Event, error) bool)) ([]agent.Event, error) {
	t.Helper()
	...
	for evt, err := range iter {
		if err != nil && firstError == nil {
			firstError = err
			// Continue collecting events that may come after error
			continue
		}

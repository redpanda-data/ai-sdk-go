...
// plotInputSchema returns the JSON Schema for the plot tool's input.
// The body is built as a map and parsed via jsonschema.Schema.UnmarshalJSON
// so we keep the human-readable map literal while exposing a typed schema
// on ToolDefinition.Parameters.
func plotInputSchema() *jsonschema.Schema {
	...
	if err != nil {
		panic(fmt.Sprintf("plot tool schema marshal: %v", err)) //nolint:forbidigo // schema literals are developer-authored; misparse must surface immediately
	}

	s := &jsonschema.Schema{}
	if err := s.UnmarshalJSON(data); err != nil {
		panic(fmt.Sprintf("plot tool schema parse: %v", err)) //nolint:forbidigo // schema literals are developer-authored; misparse must surface immediately
	}
